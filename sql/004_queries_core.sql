...
-- Baseline catalog keeps RETURNING columns stable without OLD/NEW aliases.
-- run_queries.py reads fetched_rows[0][1] (`new_version`) for version tracking.
-- `old_*` fields are intentional NULL placeholders for cross-catalog shape parity;
-- they are not consumed by automated workload logic.
...
-- Baseline catalog keeps RETURNING columns stable without OLD/NEW aliases.
-- Returned values are not consumed by run_queries.py; column layout is kept
-- consistent with the 18.x comparison catalog for manual inspection symmetry.
