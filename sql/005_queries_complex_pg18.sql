...
-- JSONB containment predicates are written as @> + ANY(array) so the
-- jsonb_path_ops GIN index can be reused while keeping value lists concise.
SELECT p.country,
       p.language,
...
-- 18.x comparison catalog keeps this CTE shape explicit to observe
-- potential planner/runtime behavior differences in newer majors.
WITH ranked AS (
  SELECT p.profile_id,
         p.country,
         p.language,
